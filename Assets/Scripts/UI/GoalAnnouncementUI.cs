...
namespace YubiSoccer.UI
{
    ...
    public class GoalAnnouncementUI : MonoBehaviour
    {
        ...
        [Tooltip("中央基準で右側(>0)へどれだけオフセットして開始するか（px）")]
        [SerializeField] private float startOffsetRight = 800f;
        [Tooltip("中央基準で左側(>0)へどれだけオフセットして終了するか（px）")]
        ...
        private void Awake()
        {
            ...
            centerAnchoredPos = goalRect.anchoredPosition;
            ...
            // 初期は画面外右に退避＆非表示
            goalRect.anchoredPosition = centerAnchoredPos + Vector2.right * startOffsetRight;
        ...
        private IEnumerator CoPlay(Team team)
        {
            ...
            goalCg.alpha = 1f;
            yield return Slide(goalRect, centerAnchoredPos + Vector2.right * startOffsetRight, centerAnchoredPos, slideInDuration);

            // 少しホールド
            if (holdDuration > 0f) yield return new WaitForSeconds(holdDuration);

            // 左へスライドアウト
            yield return Slide(goalRect, centerAnchoredPos, centerAnchoredPos + Vector2.left * endOffsetLeft, slideOutDuration);
